          ...
          {BENEFITS.map(({ icon: Icon, title, description, highlight }) => (
            <StaggerItem key={title}>
              <div className="group bg-white border border-gray-100 rounded-2xl p-8 shadow-sm hover:shadow-lg hover:-translate-y-2 transition-all duration-300">
                <div className="w-14 h-14 bg-[#0F766E]/10 rounded-xl flex items-center justify-center mb-6 group-hover:bg-[#0F766E]/20 transition-colors">
                  <Icon className="text-[#0F766E]" size={28} />
                </div>
                <h3 className="text-xl font-display font-bold text-[#1E3A5F] mb-3">{title}</h3>
                <p className="text-gray-500 leading-relaxed mb-4">{description}</p>
                <span className="inline-block bg-[#D4A843]/10 text-[#D4A843] text-xs font-semibold px-3 py-1 rounded-full">
                  {highlight}
                </span>
              </div>
            </StaggerItem>
