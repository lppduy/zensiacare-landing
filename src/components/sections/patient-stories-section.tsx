...
export function PatientStoriesSection() {
  return (
    <section id="stories" className="py-20 lg:py-28 bg-gradient-to-br from-[#0F766E]/5 to-[#1E3A5F]/5">
          ...
          {TESTIMONIALS.map(({ quote, name, location, treatment, initials }) => (
            <StaggerItem key={name}>
              <div className="bg-white rounded-2xl p-8 shadow-sm hover:shadow-lg transition-shadow duration-300 border border-gray-100 flex flex-col h-full">
                ...
                <p className="text-[#1E3A5F]/80 leading-relaxed flex-1 text-base italic mb-6">
                  &ldquo;{quote}&rdquo;
                </p>
                ...
                <div className="flex items-center gap-4 pt-4 border-t border-gray-100">
                  <div className="w-12 h-12 bg-[#1E3A5F] rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                    {initials}
                  </div>
                  <div>
                    <div className="font-display font-bold text-[#1E3A5F]">{name}</div>
